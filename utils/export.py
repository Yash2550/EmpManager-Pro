...
def export_employees_excel(employees):
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Employees" # type: ignore
    ...
    for row, emp in enumerate(employees, 2):
        ...
        for col, val in enumerate(values, 1):
            cell = ws.cell(row=row, column=col, value=val) # type: ignore
            cell.border = thin_border
    ...
    for col in ws.columns: # type: ignore
        ...
        ws.column_dimensions[col[0].column_letter].width = max(max_length + 2, 12) # type: ignore
